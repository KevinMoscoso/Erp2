...
<head>
  <meta charset="utf-8">
  <title><?= htmlspecialchars($title ?? 'Crear compra', ENT_QUOTES, 'UTF-8') ?></title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
</head>
<body>
  <h1><?= htmlspecialchars($title ?? 'Crear compra', ENT_QUOTES, 'UTF-8') ?></h1>

  <p><a href="/compras">Volver al listado</a></p>

  <?php if (!empty($error)): ?>
    <p style="color:#b00020;"><?= htmlspecialchars((string)$error, ENT_QUOTES, 'UTF-8') ?></p>
  <?php endif; ?>

  <form method="post" action="<?= htmlspecialchars((string)($action ?? '/compras/crear'), ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div style="margin-top:8px;">
      <label for="tercero_id">Proveedor</label><br>
      <select id="tercero_id" name="tercero_id" required>
        <option value="">-- Seleccionar --</option>
        <?php foreach (($proveedores ?? []) as $t): ?>
    ...
    <h2>Líneas (hasta 5)</h2>
    <p>Si eliges un producto, puedes dejar la descripción vacía.</p>
    ...
    <?php for ($i = 0; $i < 5; $i++): ?>
      <fieldset style="margin-bottom: 10px;">
        ...
        <div style="margin-top:6px;">
          ...
          <input name="line_cantidad[]" type="number" step="0.01" min="0.01" value="1" style="width: 140px;">
        </div>

        <div style="margin-top:6px;">
          <label>Costo unitario</label><br>
          <input name="line_costo_unitario[]" type="number" step="0.01" min="0" style="width: 140px;">
        </div>
      </fieldset>
    <?php endfor; ?>
